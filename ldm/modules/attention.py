...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        # x shape: torch.Size([4, 4096, 320])
        h = self.heads
        q = self.to_q(x) # q shape: torch.Size([4, 4096, 320])
        context = default(context, x) #  context shape: torch.Size([8, 77, 1024])

        k = self.to_k(context.half()) # k shape: torch.Size([4, 77, 320]) 77是n
