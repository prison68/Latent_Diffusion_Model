...
from new_module.module054 import FCR
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # 在这加载U-net，具体的U-Net还是得看unet_config
        # ldm.modules.diffusionmodules.openaimodel.UNetModel
        self.model = DiffusionWrapper(unet_config, conditioning_key).to(device='cuda:0').half()# 这里
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.m054 = FCR()
        self.zheng_yang_ben = MemoryEfficientCrossAttention(query_dim=4, context_dim=1024, heads=2).to(self.device)
    ...
    def instantiate_first_stage(self, config):
        ...
        self.first_stage_model = model.eval() # 不需要改，我不训练图像编码
    ...
    def get_first_stage_encoding(self, encoder_posterior):
        if isinstance(encoder_posterior, DiagonalGaussianDistribution):
            ...
            # print("get_first_stage_encoding:", z.shape)
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(c).to('cuda:0')
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise) # 加了噪音之后的图像
        model_output = self.apply_model(x_noisy, t, cond)
        # print('cond shape:', cond.shape) # cond shape: torch.Size([8, 77, 1024])
        # print('x shape:', x_noisy.shape) # x shape: torch.Size([8, 4, 64, 64])
        # -----------------------------------------------
        # 锚框是预测的噪音
        aa = model_output
        # 正样本是图像和文本的交叉注意力
        zz = self.zheng_yang_ben(to_3d(x_start), cond)
        # 负样本是加了噪音的图像
        bb = x_noisy
        m_loss = self.m054(aa, to_4d(zz, 64, 64), bb)
        # ------------------------------------------------
        ...
        t = t.to(device)
        logvar_t = self.logvar[t].to(device=device)
        ...
        loss = self.l_simple_weight * (loss.mean() + m_loss)
        # print('loss:', loss)
    ...
    def configure_optimizers(self): # 这里优化的模型参数就是Unet 明天再来看
        lr = self.learning_rate
        params = list(self.model.parameters())
        if self.cond_stage_trainable:
        ...
        opt = torch.optim.AdamW(params, lr=lr, eps=1e-3)
        if self.use_scheduler:
...
# 来自这 把U-Net封装在了DiffusionWrapper里
class DiffusionWrapper(pl.LightningModule):
    ...
    # 去噪默认执行的是前向传播函数，把U-Net封装在了DiffusionWrapper里
