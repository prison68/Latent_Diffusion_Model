...
@dataclass
class EvalConfig:
    output_path: Path = Path("./outputs_imgs/")
...
def run(config: EvalConfig):
    ...
    prompts = [p.name for p in config.output_path.glob("*") if p.is_dir() and not p.name.startswith('.')]
    ...
    # aggregated_results = {k: v.tolist() for k, v in aggregated_results.items()}
...
def aggregate_by_min_half(d):
    ...
    min_per_half_res = np.array(min_per_half_res, dtype=object).flatten()
...
def aggregate_by_full_text(d):
    ...
    full_text_res = np.array(full_text_res, dtype = object).flatten()
